...
export default function QuoteBox() {
  const [current, setCurrent] = useState(getRandomQuote(quotes));

  const handleNewQuote = () => setCurrent(getRandomQuote(quotes));

  const handleCopy = () => {
    navigator.clipboard.writeText(`"${current.quote}" — ${current.author || "Unknown"}`);
    alert("✅ Quote copied!");
  };
  ...
  return (
    <div className="bg-gradient-to-r from-blue-50 to-blue-100 shadow-2xl rounded-3xl p-8 w-full max-w-lg mx-auto text-center relative">
      <AnimatePresence mode="wait">
        ...
        >
          <p className="text-2xl md:text-3xl font-semibold italic text-gray-800 mb-4">“{current.quote}”</p>
          <p className="text-lg md:text-xl text-blue-700 font-medium mb-6">— {current.author || "Unknown"}</p>
        </motion.div>
      </AnimatePresence>
      ...
      <div className="flex justify-center gap-4 flex-wrap">
        <button
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-full font-semibold shadow-lg transition transform hover:-translate-y-1"
        >
          New Quote
        </button>
        <button
          onClick={handleCopy}
          className="bg-gray-700 hover:bg-gray-800 text-white px-5 py-2 rounded-full font-semibold shadow-lg transition transform hover:-translate-y-1"
        >
          Copy
        </button>
      ...
      <p className="text-sm text-gray-500 mt-6">Made with ❤️ by Omar Zouglah</p>
    </div>
  );
}
